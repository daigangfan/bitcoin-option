...
model_call=stf.ols("const_bias ~ log_ret + amihud + maxmin_ratio + btc_volume + delta_5 + vol_pre + spread + open_interest + slope ",data=calls,hasconst=True).fit()
model_put=stf.ols("const_bias ~ log_ret + amihud + maxmin_ratio + btc_volume + delta_5 + vol_pre + spread + open_interest + slope",data=puts,hasconst=True).fit()
