...
model_call=stf.ols("bias ~ log_ret + kurtosis+amihud + maxmin_ratio + btc_volume + delta + vol_pre + open_interest +time +moneyness",data=calls,hasconst=True).fit()
model_put=stf.ols("bias ~ log_ret + kurtosis+amihud + maxmin_ratio + btc_volume + delta + vol_pre + open_interest+time+moneyness",data=puts,hasconst=True).fit()
