...
results.iloc[0, results.columns.get_loc(
    "contract_label")] = "BTC 2017-10-27 Put $5600.00"
...
results["date"] = results["date"].apply(
    lambda x: datetime.strptime(x, "%Y-%m-%d"))
results["exp_date"] = results["exp_date"].apply(
    lambda x: datetime.strptime(x, "%Y-%m-%d"))
...
results["time"] = results.apply(get_time, axis=1)

writer = pd.ExcelWriter("new_data/ledgerx_data.xlsx")
with writer:
    results.to_excel(writer,index=False)

# get bitcoin data from coinmarketcap,move all preprocessing for btc_data here
page = sess.get(
    "https://coinmarketcap.com/currencies/bitcoin/historical-data/?start=20160801&end=20190414")
if page.status_code == 200:
    ...
    btc_data.rename(
        columns={"Open*": "Open", "Close**": "Price"}, inplace=True)
