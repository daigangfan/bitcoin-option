...
model_2=stf.ols('''bias ~ log_ret + skewness + amihud + maxmin_ratio + btc_volume + 
    delta + vol_pre + open_interest +time+ contract_is_call + 
    inter_call_money + inter_put_money''',data=used_data,hasconst=True).fit()
model_3=stf.ols(
    '''bias ~ log_ret + volatility + amihud + maxmin_ratio + btc_volume + 
    delta + vol_pre + open_interest +time+ contract_is_call + 
    inter_call_money + inter_put_money''',data=used_data,hasconst=True
).fit()
summaries = summary_col([model_1_stepwise,model_2,model_3], stars=True, model_names=["(1)","(2)","(3)"],info_dict={
...
with open("drift/new_describes/regression_table_other_var.tex", "w",encoding="utf-8") as f:
